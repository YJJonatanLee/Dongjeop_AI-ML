...
show_help() {
    ...
    echo ""
    echo "  # 파인튜닝된 모델로 추론 (linear head)"
    echo "  $0 --image test.jpg --checkpoint models/checkpoints/siglip-base-patch16-224/best_model.pth"
    echo ""
    echo "  # ML Decoder 모델로 추론 (자동 감지)"
    echo "  $0 --image test.jpg --checkpoint models/checkpoints/siglip-base-patch16-224-mldecoder/best_model.pth"
    echo ""
    echo "  # 임계값 조정"
